...
class ResultsScreen:
    ...
    def draw(self, surface):
        ...
        # High score line + NEW RECORD badge — brighter so it reads at a
        # glance from the same distance as the score.
        if self.new_record:
            draw_text(surface, "★ NEW HIGH SCORE ★",
                      FONT_SM, AMBER, cx, 326, bold=True)
        else:
            draw_text(surface, f"Best:  {self.high:,}",
                      FONT_SM, WHITE, cx, 326, bold=True)

        mode_label = {"space": "Space Mode",
                      "quiz":  "Quiz Mode"}.get(self.mode, self.mode.title())
        combo_label = (f"  ·  Best combo: x{self.best_combo}"
                       if self.best_combo >= 2 else "")
        draw_text(surface, f"Mode: {mode_label}{combo_label}",
                  FONT_SM, WHITE, cx, 354, bold=True)
    ...
    def _draw_stats_panel(self, surface, cx: int):
        ...
        The panel is sized and positioned based on the actual space between
        the Mode line above and the button row below, so it never overlaps
        the buttons regardless of window height.
        """
        top_anchor   = 386                                    # below Mode line
        bot_anchor   = self.btn_again.rect.top - 30           # above buttons
        avail_h      = max(140, bot_anchor - top_anchor)
        panel_h      = min(280, avail_h)
        # Centre the panel in the available band so the result looks
        # balanced at any window size.
        panel_top    = top_anchor + (avail_h - panel_h) // 2
        panel = pygame.Rect(cx - 380, panel_top, 760, panel_h)
        ...
        if not self.cat_stats:
            draw_text(surface,
                      ...
                      FONT_XS, WHITE, panel.centerx, panel.centery)
            return

        # Determine how many rows actually fit inside the panel — leaves
        # room for the header at the top and the "more" line at the bottom.
        row_h           = 28
        content_top     = panel.top + 58
        content_bottom  = panel.bottom - 30      # reserve space for footer
        max_visible     = max(1, (content_bottom - content_top) // row_h)
        rows            = self.cat_stats[:max_visible]
        ...
        remaining = len(self.cat_stats) - len(rows)
        if remaining > 0:
            draw_text(surface,
                      f"+ {remaining} more — open REVIEW for the full list.",
                      FONT_XS, WHITE, panel.centerx, panel.bottom - 18)
