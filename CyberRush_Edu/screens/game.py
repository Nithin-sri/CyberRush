...
class GameScreen:
    ...
    def _init(self):
        ...
        # shows the deep explanation. SPACE skips early.
        self.pause_on_hit_frames = 0
    ...
    def update(self, dt: float):
        ...
        # Only the shake-decay and pause-countdown tick while paused so
        # the visible state matches the explanation on screen.
        if self.pause_on_hit_frames > 0:
            self.pause_on_hit_frames -= 1
            ...
            if not obs.collected and self.player.rect.colliderect(obs.rect):
                ...
                if obs.label == "phishing":
                    ...
                    if self.player.hit():
                        ...
                        # actually reads the explanation.
                        self.encountered.append(
                            {"question": obs.question, "outcome": "hit"})
                        self.pause_on_hit_frames = WHY_PAUSE_FRAMES
    ...
    def handle_event(self, event) -> str | None:
        if event.type == pygame.KEYDOWN:
            # SPACE skips the rest of the "Why?" pause if it's active
            if (event.key == pygame.K_SPACE
                    and self.pause_on_hit_frames > 0):
                self.pause_on_hit_frames = 0
                return None
    ...
    def draw(self, surface):
        ...
        if self.pause_on_hit_frames > 0 and self.pause_on_hit_data:
    ...
    def _draw_why_overlay(self, surface):
        ...
        # Countdown + skip hint
        secs_left = max(0, int((self.pause_on_hit_frames + FPS - 1) / FPS))
        draw_text(surface,
                  f"Resuming in {secs_left}s   ·   press SPACE to continue",
