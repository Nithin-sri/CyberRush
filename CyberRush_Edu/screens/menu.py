...
class MenuScreen:
    ...
    def _draw_title(self, surface, cx, base_y):
        # Animated pulse + soft glow halo
        pulse = 1 + 0.025 * math.sin(self.time * 2.4)
        size  = int(FONT_HUGE * pulse * 0.85)
        font  = get_font(size, bold=True)

        # Glow layers — multiple offsets with low alpha for a neon look
        for offset, alpha in [(6, 30), (3, 60), (1, 110)]:
            glow = font.render("CYBERRUSH", True, CYAN)
        ...
        # Main title
        title = font.render("CYBERRUSH", True, CYAN)
        surface.blit(title, (cx - title.get_width() // 2, base_y))

        # Underline accent — animated cyan bar under the title
        bar_w = int(title.get_width() * 0.55)
        bar_y = base_y + title.get_height() + 4
        pygame.draw.rect(surface, CYAN,
                         (cx - bar_w // 2, bar_y, bar_w, 3),
                         border_radius=2)
    ...
    def draw(self, surface):
        ...
        # High-score line — shows whichever mode is higher (or both if any exist)
        hs_space = highscore.get("space")
        hs_car   = highscore.get("car")
        if hs_space or hs_car:
            parts = []
            if hs_space: parts.append(f"SPACE  {hs_space:,}")
            if hs_car:   parts.append(f"CAR  {hs_car:,}")
