...
def test_game_pause_on_phishing_hit():
    from screens.game import GameScreen
    from settings     import WHY_PAUSE_FRAMES
    g = GameScreen(800, 600)
    g.set_mode("space")
    # Fresh game starts with empty encountered list and no forced pause
    assert g.encountered == []
    assert g.pause_on_hit_frames == 0
    # WHY_PAUSE_FRAMES is the freeze duration used after a phishing hit
    assert WHY_PAUSE_FRAMES > 0
