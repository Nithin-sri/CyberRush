...
class Player:
    ...
    # Tunable constants now live in settings.py
    ...
    def handle_input(self, keys):
        if self.mode == "space":
            ...
            if self.lane_cooldown > 0:
                ...
                return
            from utils.sound import get_sounds   # lazy: avoids import cycles
            if (keys[pygame.K_LEFT] or keys[pygame.K_a]) and self.lane > 0:
                self.lane -= 1
                self._set_lane_x()
                self.lane_cooldown = LANE_COOLDOWN_FRAMES
                get_sounds().play("lane")
